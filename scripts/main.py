...
from scripts.ensemble.features import AllInOneModel, PerLabelModel
from scripts.ensemble.models import (
    BinaryEnsemble,
    Ensemble,
    IsolatedDualEnsemble,
    MultiScenarioSKEmsemble,
    MultiSourceEnsemble,
    SklearnEnsemble,
)
from scripts.ensemble.selectors import (
    AcceptLabelIfExpertVoted,
    AcceptLabelWithGoldOracle,
    KeepTopKSubmissions,
    ScoreLabelAsMaxVoteWeight,
    SetParameterAsThreshold,
    WeightVotesWithF1Score,
)
from scripts.utils import ENTITIES, RELATIONS, Collection, Keyphrase, Relation, Sentence


def task_run(e: Ensemble, ps, pg, best):
...
if __name__ == "__main__":
    ...
    # e = Ensemble()
    # e = BinaryEnsemble()
    # e = KeepTopKSubmissions(Ensemble(), 1)
    # e = KeepTopKSubmissions(BinaryEnsemble(), 1)
    # e = WeightVotesWithF1Score(BinaryEnsemble())
    # e = ScoreLabelAsMaxVoteWeight(WeightVotesWithF1Score(Ensemble()))
    # e = AcceptLabelIfExpertVoted(WeightVotesWithF1Score(Ensemble()))
    # e = AcceptLabelIfExpertVoted(WeightVotesWithF1Score(BinaryEnsemble()))
    # e = AcceptLabelWithGoldOracle(WeightVotesWithF1Score(Ensemble()))
    # e = AcceptLabelWithGoldOracle(WeightVotesWithF1Score(BinaryEnsemble()))
    # e = SklearnEnsemble()
